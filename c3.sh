#!/bin/bash
# =============================================================================
# ASID 2025/2026 — C3: Escalamento Uniforme
# Todos os serviços stateless com 3 réplicas (redis-cart mantém 1)
# Execução COMPARATIVA: cargas fixas 15/20/25 users, warm-up 30s + medição 120s
# =============================================================================
NAMESPACE="${NAMESPACE:-default}"
FRONTEND_URL="${FRONTEND_URL:-http://localhost}"
RESULTS_DIR="cenarios_ob_c3"
...
# Serviços stateless a escalar (redis-cart fica com 1 réplica — stateful)
STATELESS_SVCS="adservice cartservice checkoutservice currencyservice emailservice frontend paymentservice productcatalogservice recommendationservice shippingservice"
...
scale_all_stateless() {
  log "=== C3: A escalar todos os serviços stateless → 3 réplicas ==="
  for svc in $STATELESS_SVCS; do
    kubectl -n "$NAMESPACE" scale deployment/$svc --replicas=3 2>/dev/null && \
      echo "  $svc → 3" || echo "  AVISO: $svc não encontrado"
  done
  log "A aguardar que todos fiquem prontos..."
  for svc in $STATELESS_SVCS; do
    kubectl -n "$NAMESPACE" rollout status deployment/$svc --timeout=120s 2>/dev/null
  done
  log "redis-cart mantém 1 réplica (stateful)"
  kubectl -n "$NAMESPACE" get deployments | grep -v "^NAME" | awk '{print "  "$1": "$2" réplicas"}'
}
...
stop_monitoring() {
  [ -f "$RESULTS_DIR/monitor.pid" ] && kill "$(cat "$RESULTS_DIR/monitor.pid")" 2>/dev/null
...
LOCUST_EOF
  log "locustfile.py criado (3 perfis — idêntico a C1comp e C2)"
}
...
run_fixed_load() {
  ...
  log "========================================================"
  log "C3 — ${users} users (warm-up ${WARMUP_DURATION}s + medição ${MEASURE_DURATION}s)"
  log "========================================================"
...
generate_report() {
  local report="$RESULTS_DIR/RESUMO_C3.txt"
  python3 - "$RESULTS_DIR" << 'PYEOF' > "$report"
import csv, os, sys
results_dir = sys.argv[1]
lines = []
lines.append("=" * 80)
lines.append("ASID 2025/2026 — C3: Escalamento Uniforme (todos stateless × 3)")
...
lines.append("")
lines.append("CPU/RAM por pod (após cada nível de carga)")
lines.append("-" * 70)
for users in [15, 20, 25]:
...
main() {
  log "=============================================="
  log "C3 — ESCALAMENTO UNIFORME"
  log "Todos os stateless: 3 réplicas | redis-cart: 1"
