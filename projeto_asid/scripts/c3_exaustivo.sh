#!/bin/bash
# =============================================================================
# ASID 2025/2026 — C3 Exaustivo
# Todos os 10 stateless: 3 réplicas; redis-cart: 1 réplica
# Step-up 30→70 users (passo 5) até p99>2000ms ou falhas>5%
# =============================================================================
# COMO USAR:
#   chmod +x c3_exaustivo.sh
#   ./c3_exaustivo.sh
#   # ou em background:
#   nohup ./c3_exaustivo.sh > c3_exaustivo.log 2>&1 &
#   tail -f c3_exaustivo.log
# =============================================================================

NAMESPACE="${NAMESPACE:-default}"
FRONTEND_URL="${FRONTEND_URL:-http://localhost}"
RESULTS_DIR="cenarios_ob_c3_exaustivo"
...
# =============================================================================
set_replicas() {
  log "=== C3: todos stateless → 3 réplicas; redis-cart mantém 1 ==="
  ...
  kubectl -n "$NAMESPACE" rollout status deployment/frontend --timeout=120s 2>/dev/null
  log "C3 configurado (todos stateless ×3; redis-cart ×1)"
...
run_load_level() {
  ...
  log "========================================================"
  log "C3 Exaustivo — ${users} users (warm-up ${WARMUP_DURATION}s + medição ${MEASURE_DURATION}s)"
  log "========================================================"
...
# =============================================================================
generate_report() {
  local report="$RESULTS_DIR/RESUMO_C3_exaustivo.txt"
  python3 - "$RESULTS_DIR" "$BREAK_P99" "$BREAK_FAIL_PCT" << 'PYEOF' | tee "$report"
import csv, os, sys
d, bp99, bfail = sys.argv[1], sys.argv[2], sys.argv[3]
print("=" * 70)
print("ASID 2025/2026 — C3 Exaustivo (todos stateless ×3; redis-cart ×1)")
...
main() {
  log "=============================================="
  log "C3 EXAUSTIVO — todos stateless ×3"
  ...
  log ""
  log "=============================================="
  log "C3 Exaustivo CONCLUÍDO — $RESULTS_DIR"
  log "=============================================="
}
