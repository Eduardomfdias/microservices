...
\textbf{RQ3} — O ganho não justifica o custo sem balanceamento L7. C2: $CM_{tp}=-12{,}15$~RPS/réplica; C4: $CM_{tp}=-8{,}40$~RPS/réplica; C3: $CM_{tp}=+0{,}44$~RPS/réplica (+67\% custo/req). Com Envoy: C5 tem $CM_{tp}=+5{,}63$~RPS/componente e custo/req 18\% abaixo do C1, demonstrando que o balanceamento L7 inverte o sinal dos rendimentos marginais.

\textbf{RQ4} — O \texttt{redis-cart} não foi fator limitante nas cargas testadas (máx.\ 18~m CPU a 150u no C5). H2 e H5 permanecem inconclusivas, dado que cargas superiores seriam necessárias para observar a limitação stateful.
