...
% --- Resumo ---
\begin{abstract}
Este artigo estuda o impacto de diferentes estratégias de escalabilidade horizontal sobre o desempenho e o custo marginal de uma aplicação de microserviços, utilizando o Google Online Boutique como sistema em estudo. Cinco cenários experimentais foram desenhados para comparar configurações de escalamento progressivamente distintas: C1 (baseline, 1 réplica), C2 (escalamento seletivo do candidato a \textit{bottleneck}), C3 (escalamento uniforme de todos os serviços \textit{stateless}), C4 (escalamento dos bottlenecks identificados empiricamente) e C5 (escalamento uniforme com proxy Envoy para balanceamento L7). Foram executados testes comparativos (15--25 utilizadores) e testes exaustivos (25--150+ utilizadores) com o Locust, repetidos três vezes por cenário. Os resultados revelam três efeitos contra-intuitivos: (1) C2 atingiu saturação a 50 utilizadores, antes do C1 a 75 utilizadores — o escalamento seletivo mal direcionado degradou o sistema por via da \textit{connection affinity} do gRPC/HTTP2; (2) C4, ao escalar os bottlenecks reais identificados, produziu o pior resultado de todos, com 77,1\% de falhas a 25 utilizadores, por efeito de cascata nas dependências não escaladas; (3) C5, com balanceamento por pedido via Envoy, atingiu 218,7~RPS a 150 utilizadores sem falhas, com custo por pedido 18\% \textit{inferior} ao baseline — invertendo o padrão de custo marginal crescente observado em C2 e C3. Os resultados suportam fortemente a hipótese de que a \textit{connection affinity} do gRPC/HTTP2 é o fator limitante dominante do escalamento horizontal em Kubernetes, e que o balanceamento ao nível de pedido (L7) é condição necessária para que o scale-out produza ganhos proporcionais.
\end{abstract}
