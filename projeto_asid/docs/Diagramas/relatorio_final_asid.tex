...
% ============================================================
\subsection{Decisão Arquitetural em Estudo}
% ============================================================
...
\subsubsection{Alternativas Consideradas à Decisão de Escalamento Estático}
...
\subsubsection{Trade-offs: Escalamento Seletivo vs. Uniforme}
...
\midrule
Performance & Ganho na latência mediana ($-22\%$ p50). Risco: se o bottleneck não for o serviço escalado, pode degradar (C2 $\approx$ C1 nos exaustivos) & Maior ponto de saturação (150u vs 75u). Sem melhoria na mediana. \\
\addlinespace
...
\begin{table}[h]
\caption{Resultados exaustivos C1 (1 réplica, médias). Quebra a 75 utilizadores com 23,0\% de falhas.}
\label{tab:c1ex}
\begin{tabular}{lrrrrrr}
\toprule
\textbf{Users} & \textbf{p50} & \textbf{p90} & \textbf{p99} & \textbf{Falhas\%} & \textbf{RPS} & \textbf{Estado} \\
...
\begin{table}[h]
\caption{Resultados exaustivos C2 (productcatalogservice ×3, médias). Quebra a 75 utilizadores, mesmo limite do C1.}
\label{tab:c2ex}
\begin{tabular}{lrrrrrr}
\toprule
\textbf{Users} & \textbf{p50} & \textbf{p90} & \textbf{p99} & \textbf{Falhas\%} & \textbf{RPS} & \textbf{Estado} \\
\midrule
25 & 9ms & 30ms & 395ms & 0,0\% & 86,4 & OK \\
50 & 88ms & 185ms & 765ms & 0,0\% & 117,7 & OK \\
\textbf{75} & \textbf{115ms} & \textbf{260ms} & \textbf{1010ms} & \textbf{33,6\%} & \textbf{151,2} & \textbf{QUEBRA} \\
\bottomrule
\end{tabular}
\end{table}

\begin{table}[h]
\caption{Resultados exaustivos C3 (todos stateless ×3). Suporta 400 utilizadores sem quebrar.}
\label{tab:c3ex}
\begin{tabular}{lrrrrrr}
\toprule
\textbf{Users} & \textbf{p50} & \textbf{p90} & \textbf{p99} & \textbf{Falhas\%} & \textbf{RPS} & \textbf{Estado} \\
...
\begin{table}[h]
\caption{Resultados exaustivos C4 (frontend ×3 + currencyservice ×3, médias). Suporta 400 utilizadores sem quebrar, sucesso do escalamento seletivo direcionado aos bottlenecks reais.}
\label{tab:c4ex}
\begin{tabular}{lrrrrrr}
\toprule
\textbf{Users} & \textbf{p50} & \textbf{p90} & \textbf{p99} & \textbf{Falhas\%} & \textbf{RPS} & \textbf{Estado} \\
...
C2 — Seletivo (productcatalog ×3)    & +2      & \textbf{75u}     & 151,2 & 33,6 & Igual a C1 (em utilizadores) \\
...
Este efeito cascata ilustra uma propriedade estrutural das arquitecturas de microsserviços com gRPC: \textbf{escalar qualquer serviço intermediário multiplica a carga de conexões nos seus dependentes}. Sem escalar a cadeia completa de dependências, o bottleneck apenas se desloca — não desaparece. O C3, ao escalar todos os serviços stateless uniformemente, é o único cenário que evita este efeito.

\subsection{O Resultado Mais Surpreendente: C2 $\approx$ C1}

O resultado mais notório deste estudo é o facto de o C2 (escalamento seletivo com +2 réplicas) ter atingido saturação ao mesmo tempo que o C1 (baseline sem escalamento). Este resultado, aparentemente paradoxal, é explicado por dois mecanismos que se reforçam mutuamente:
...
\textbf{C2 — Escalamento seletivo (productcatalogservice ×3).} Resultado contra-intuitivo: quebra a 75u, \emph{igual} ao baseline. As réplicas adicionais do \texttt{productcatalogservice} absorveram apenas 2\% do tráfego (97,9\% permaneceu na réplica original, conforme ilustrado na Figura~\ref{fig:distrib-cpu}), mas consumiram RAM e CPU do nó, degradando os serviços realmente pressionados. Este resultado é consistente com H6 e representa a evidência empírica mais forte do estudo.

\textbf{C3 — Escalamento uniforme (todos stateless ×3).} O único cenário com $CM_{tp}$ positivo ($+0{,}44$~RPS/réplica): suporta 400u sem falhas. O custo por pedido sobe 67\% (exaustivos) e 130\% (comparativos). A fração serial efetiva de 41,5\% (Lei de Amdahl) mostra que o teto teórico é $\approx$1,64$\times$. Escalar além de 3 réplicas produziria ganhos marginais ínfimos.

\textbf{C4 — Escalamento dos bottlenecks identificados (frontend ×3 + currencyservice ×3).} Um dos melhores resultados: escalamento cirúrgico dos gargalos com capacidade até 400u sem falhas. Escalar o \texttt{frontend} (ponto de entrada HTTP/1.1) triplicou as conexões gRPC nos backends não escalados: o \texttt{cartservice} atingiu 115~m de CPU (Figura~\ref{fig:cpu-servicos}), tornando-se o novo bottleneck. O C4 demonstra que identificar e escalar os serviços mais pressionados (frontend, currency) é uma estratégia válida quando bem executada, sem necessariamente despoletar colapsos incontroláveis nas dependências em certas arquiteturas.

\textbf{C5 — Escalamento uniforme ×3 + Envoy L7.} Único cenário sem quebra a 150u: 218,7~RPS, 0\% de falhas. O custo por pedido de 5,34 unidades é 18\% \emph{abaixo} do C1, porque o throughput elevado dilui o custo total. A fração serial efetiva (Lei de Amdahl) baixou de 65\% (C3) para 19\% (C5), elevando o teto teórico de 1,53$\times$ para 5,32$\times$. O C5 confirma que a \textit{connection affinity} era o factor limitante dominante, e não a capacidade de computação agregada.
...
\textbf{RQ2} — Depende do alvo: o C2 degradou a eficiência, mas o C4 escalou com sucesso (\textit{H1 refutada: o escalamento seletivo bem direcionado resulta}). Com Envoy (C5), as mesmas 20 réplicas do C3 atingiram 218{,}7~RPS sem falhas — 68\% mais do que o C3. A questão relevante não é qual serviço escalar, mas como garantir que as réplicas recebem tráfego (\textit{H6 fortemente suportada}).
...
\textbf{Performance:} C3 e C4 melhoram massivamente a saturação para $>$400u; C5 (com Envoy) otimiza o uso L7 e atinge 218,7 RPS sem quebra a 150u. O escalamento seletivo mal direcionado (C2) afeta negativamente o custo marginal.
...
Em sistemas gRPC/HTTP2, o \texttt{kubectl scale} simples é uma ferramenta de eficácia limitada e potencialmente prejudicial sem balanceamento ao nível de pedido. O C5 demonstra empiricamente que um proxy L7 (Envoy com LEAST\_REQUEST) resolve estruturalmente este problema: a fração serial efetiva baixou de 65\% para 19\%, o throughput aumentou 2,18$\times$ face ao C1 e o custo por pedido ficou abaixo do baseline. Em arquiteturas microsserviços com gRPC, a decisão de escalamento horizontal deve incluir a estratégia de balanceamento como variável de primeira classe — não como detalhe de implementação.

\subsection{Limitações e Trabalho Futuro}

Os resultados foram obtidos num ambiente local de nó único, com as métricas a refletirem médias de execuções múltiplas recentes, conferindo robustez extra. A estabilidade no C3 confirma os ganhos com \textit{runs} independentes. Trabalho futuro prioritário: (1) teste do C5 além de 150u para determinar o seu verdadeiro ponto de saturação; (2) carga agressiva até colapsar o \texttt{redis-cart} com Envoy (H2/H5 inconclusivas); (3) avaliação de HPA nativo (Horizontal Pod Autoscaler) sobre o sistema com \textit{proxy} L7 para ajustar autonomamente ao tráfego real.
...
\begin{figure}[h]
  ...
  \caption{Diagrama de dependências entre microsserviços. Verde: \texttt{frontend} (depende de 8 serviços, mais vulnerável a falhas em cascata). Vermelho: \texttt{checkoutservice} (orquestrador de 6 serviços em série). Laranja: \texttt{redis-cart} (único componente \textit{stateful}). Folhas (verde claro) — \texttt{paymentservice}, \texttt{emailservice}, \texttt{adservice}, \texttt{shippingservice} — sem dependências externas. O \texttt{productcatalogservice} e o \texttt{redis-cart} são os candidatos principais a pontos de saturação: o primeiro por fan-out ×6 em W1; o segundo por contenção de estado partilhado em W2/W3.}
