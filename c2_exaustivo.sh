#!/bin/bash
# =============================================================================
# ASID 2025/2026 — C2 Exaustivo
# productcatalogservice: 3 réplicas; restantes: 1 réplica
# Step-up 30→70 users (passo 5) até p99>2000ms ou falhas>5%
# =============================================================================
# COMO USAR:
#   chmod +x c2_exaustivo.sh
#   ./c2_exaustivo.sh
#   # ou em background:
#   nohup ./c2_exaustivo.sh > c2_exaustivo.log 2>&1 &
#   tail -f c2_exaustivo.log
# =============================================================================

NAMESPACE="${NAMESPACE:-default}"
FRONTEND_URL="${FRONTEND_URL:-http://localhost}"
RESULTS_DIR="cenarios_ob_c2_exaustivo"
...
# =============================================================================
set_replicas() {
  log "=== C2: productcatalogservice → 3 réplicas; restantes → 1 ==="
  for svc in adservice cartservice checkoutservice currencyservice emailservice \
             frontend paymentservice recommendationservice shippingservice; do
    kubectl -n "$NAMESPACE" scale deployment/$svc --replicas=1 2>/dev/null
  done
  kubectl -n "$NAMESPACE" scale deployment/productcatalogservice --replicas=3 2>/dev/null
  kubectl -n "$NAMESPACE" rollout status deployment/productcatalogservice --timeout=90s 2>/dev/null
  log "C2 configurado (productcatalogservice ×3)"
...
LOCUST_EOF
  log "locustfile.py criado (3 perfis)"
}
...
run_load_level() {
  ...
  log "========================================================"
  log "C2 Exaustivo — ${users} users (warm-up ${WARMUP_DURATION}s + medição ${MEASURE_DURATION}s)"
  log "========================================================"
...
PYEOF
  ...
  local p50 p90 p99 fails failpct rps estado
...
main() {
  log "=============================================="
  log "C2 EXAUSTIVO — productcatalogservice ×3"
  ...
  log ""
  log "=============================================="
  log "C2 Exaustivo CONCLUÍDO — $RESULTS_DIR"
  log "=============================================="
}
