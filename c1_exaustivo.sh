#!/bin/bash
# =============================================================================
# ASID 2025/2026 — C1 Exaustivo
# 1 réplica por serviço (baseline)
# Step-up 30→70 users (passo 5) até p99>2000ms ou falhas>5%
# =============================================================================
# COMO USAR:
#   chmod +x c1_exaustivo.sh
#   ./c1_exaustivo.sh
#   # ou em background:
#   nohup ./c1_exaustivo.sh > c1_exaustivo.log 2>&1 &
#   tail -f c1_exaustivo.log
# =============================================================================

NAMESPACE="${NAMESPACE:-default}"
FRONTEND_URL="${FRONTEND_URL:-http://localhost}"
RESULTS_DIR="cenarios_ob_c1_exaustivo"
...
start_monitoring() {
  ...
  (
    ...
    while true; do
      ts=$(date +%H:%M:%S)
      # Lê o label corrente (escrito por mark_load_start)
...
# =============================================================================
set_replicas() {
  log "=== C1: a garantir 1 réplica em todos os serviços ==="
...
run_load_level() {
  ...
  log "========================================================"
  log "C1 Exaustivo — ${users} users (warm-up ${WARMUP_DURATION}s + medição ${MEASURE_DURATION}s)"
  log "========================================================"
  ...
  # Parse e avalia critério de quebra
...
PYEOF
  ...
  # Guardar linha no CSV de resumo
...
main() {
  log "=============================================="
  log "C1 EXAUSTIVO — 1 réplica, locustfile 3 perfis"
  ...
  log ""
  log "=============================================="
  log "C1 Exaustivo CONCLUÍDO — $RESULTS_DIR"
  log "=============================================="
}
